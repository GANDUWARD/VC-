...
int swap_edges(Edge edges[],MGraph*M)
{
    Edge t;
    for (int i = 0; i < M->arcNum - 1; i++) //为边排序
        for (int j = 0; j < M->arcNum - i - 1; j++)
        {
...
int Init_arrays(MGraph* M, int father[], Edge edges[])
{   
    int k = 0;
    for (int i = 0; i < M->vexNum; i++)//初始化子树根节点
        father[i] = i;
    for (int i = 0; i < M->vexNum; i++)      //为edges读入所有边的信息
        for (int j = i; j < M->vexNum; j++)//访问上三角，便于规定小号是起点，大号是终点
        {
            if (M->arcs[i][j] != 999 && i != j)
            {   
                edges[k].flag = 0;
                edges[k].a = i; edges[k].b = j;
                edges[k].weight = M->arcs[i][j];
                k++;
            }
        }
    return 1;
}
int Kruskal(MGraph* M, int father[], Edge edges[])
{   
    /*int father[VNUM] = { 0 }; Edge* edges = (Edge*)malloc(sizeof(Edge) * M->arcNum);*/
    int k = 0;
    Init_arrays(M, father, edges);//初始化father数组和edges数组
    swap_edges(edges, M);//为边排序
    for (int i = 0; i < M->arcNum; i++)
    {   
        if (k == M->vexNum - 1)break;//如果到达最小生成树边数，就终止遍历
        if (edges[i].b == father[edges[i].b] && edges[i].a == father[edges[i].a])//说明边ab结点都未被选上
        {
            father[edges[i].b] = edges[i].a; edges[i].flag = 1;
            k++; continue;
        }
        if (edges[i].b != father[edges[i].b] || edges[i].a != father[edges[i].a])//说明边ab结点有一点被选过了
        {   
            int  a1=father[edges[i].a], a2=father[edges[i].b],at1=father[a1],at2=father[a2];  //at表示a的下一回溯值
            while (a1!=at1||a2!=at2)//一直循环到两个端点所在子树的根节点
            {
                a1 = at1; at1 = father[at1];
                a2 = at2; at2 = father[at2];
            }
            if (a1 != a2)  //说明属于两个不同子树，合并
            {
                father[edges[i].a] = edges[i].b; edges[i].flag = 1;
                k++; continue;
            }
        }
    }
...
int main()
{
    ...
    while (1)
    {
        ...
        printf("邻接矩阵：\n");
        output_matrix(M);
        printf("\n请输入你的选择1 寻找初始点到某点的路径2 初始点到某点的最短路径3 普利姆算法求最小生成树4 深度遍历5 广度遍历6 克鲁斯卡尔算法求最小生成树\n");
        scanf_s("%d", &num);
        if (num == 1)
        {
        ...
        {
            int father[VNUM] = { 0 }; Edge* edges = (Edge*)malloc(sizeof(Edge) * M->arcNum);
            Kruskal(M, father, edges);
            int k = 0;
            for (int i = 0; i < M->arcNum; i++)
            {  
                if (edges[i].flag == 1)
                {
                    printf("%s边的权是%d,邻接点是%s\n", M->vex[edges[i].a].string, edges[i].weight, M->vex[edges[i].b].string);
                    k++;
                }
                if (k == M->vexNum - 1)break;
            }
        }
        printf("按任意键继续!\n");
        getchar(); getchar();
